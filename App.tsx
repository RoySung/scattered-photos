...
const App: React.FC = () => {
  ...
  const [background, setBackground] = useState<BackgroundSettings>({ type: 'default', value: '' });
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      return new Promise((resolve) => {
        ...
        reader.onload = (e) => {
          ...
          img.onload = () => {
            ...
            const availableWidth = Math.max(containerWidth - CARD_WIDTH - PADDING * 2, 100);
            const availableHeight = Math.max(containerHeight - CARD_HEIGHT - PADDING * 2, 100);
    ...
    setPhotos((prev) => {
      const currentMaxZ = prev.length > 0 ? Math.max(...prev.map(p => p.zIndex)) : 0;
  ...
  const handleExportImage = async () => {
    ...
    try {
      ...
      alert('Failed to generate image. If using a custom background URL, ensure it supports CORS.');
    }
  };
